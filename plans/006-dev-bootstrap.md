...
**Deviations from the official pack (per D11):**
1. `zitadel-login` service is removed entirely.
2. Traefik router labels under `zitadel-api` that exist only to coexist with Login are simplified: the `!PathPrefix(`/ui/v2/login`)` and `!Path(`/`)` exclusions on the API canonical router are removed, and the `zitadel-root-rewrite` middleware + all `zitadel-root-*` and `zitadel-login-*` routers are dropped. The API catch-all now handles everything on `${ZITADEL_DOMAIN}`.
...
4. **Added** `ZITADEL_FIRSTINSTANCE_ORG_MACHINE_*` env vars (per D12) so Zitadel provisions an admin machine user and writes its PAT to `/zitadel/bootstrap/pim-admin.pat` on the shared `zitadel-bootstrap` volume. `just dev-up` will later copy that PAT value into `.env.local` as `ZITADEL_ADMIN_PAT`.
5. `ZITADEL_DOMAIN` is set to `pim.localhost` (per D7). `PROXY_HTTP_PUBLISHED_PORT=8080` keeps the user-visible URL as `http://pim.localhost:8080`.
...
- [ ] **Step 1: Derive and write `compose.yml`**

Copy the upstream `docker-compose.yml` as a starting point, apply the six deviations above, and add the two PIM services. Expected top-level shape:

```yaml
name: pim

services:
  proxy:          # verbatim from upstream, minus the zitadel-login depends_on
  zitadel-api:    # upstream minus LOGINCLIENT/LOGINV2 vars, plus FIRSTINSTANCE_ORG_MACHINE_* vars
...
**Key `zitadel-api` environment additions** (the FIRSTINSTANCE_ORG_MACHINE_* exact names must be verified against `cmd/defaults.yaml` in the zitadel repo; names shown are the best-effort mapping):

```yaml
# Dispense a PAT for pim-bootstrap at first boot.
ZITADEL_FIRSTINSTANCE_PATPATH: /zitadel/bootstrap/pim-admin.pat
ZITADEL_FIRSTINSTANCE_ORG_MACHINE_MACHINE_USERNAME: pim-bootstrap
ZITADEL_FIRSTINSTANCE_ORG_MACHINE_MACHINE_NAME: Pim Bootstrap Admin
ZITADEL_FIRSTINSTANCE_ORG_MACHINE_PAT_EXPIRATIONDATE: "2099-01-01T00:00:00Z"
```

> If the env var names above don't match `defaults.yaml`, fall back to providing a `steps.yaml` file via `--steps` flag on `start-from-init` and defining the admin machine + PAT there. The verification step below will catch schema mismatches early.

- [ ] **Step 2: Drop the stale `docker-compose.yml`**

```bash
git rm docker-compose.yml
```

- [ ] **Step 3: Add `pim.localhost` loopback alias**

Document in `docs/dev-bootstrap.md` that `pim.localhost` resolves to `127.0.0.1` automatically on Linux/macOS per RFC 6761. No `/etc/hosts` edit needed. Verify: `getent hosts pim.localhost` (Linux) or `dscacheutil -q host -a name pim.localhost` (macOS).

- [ ] **Step 4: Smoke-test boot (manual)**

```bash
# Generate masterkey (anticipating Phase 5 justfile logic)
ZITADEL_MASTERKEY="$(openssl rand -base64 32 | tr -d '=+/' | cut -c1-32)"
export ZITADEL_MASTERKEY

podman compose up -d postgres zitadel-api
# Wait for health
podman compose ps
podman compose logs zitadel-api | grep -m1 'server is listening'

# Confirm admin PAT was dispensed
podman compose exec zitadel-api cat /zitadel/bootstrap/pim-admin.pat

# Confirm Zitadel health endpoint over Traefik
curl -sf http://pim.localhost:8080/debug/healthz

podman compose down  # leaves volumes intact
```

The PAT file content is what `just dev-up` will persist into `.env.local` as `ZITADEL_ADMIN_PAT` in Phase 5.
...
git add compose.yml
git rm docker-compose.yml
git commit -m "feat(compose): local Zitadel stack from official pack (no Login)"
```
...
- [ ] `/zitadel/bootstrap/pim-admin.pat` inside the `zitadel-api` container contains a non-empty PAT (dispensed by FIRSTINSTANCE_ORG_MACHINE_*)
...
| Phase | Status |
|-------|--------|
| Phase 1: Local compose stack | Not Started |
